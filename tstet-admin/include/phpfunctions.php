...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="category" && $field=="now")
	{
		return now();
	}
				if($table=="category" && $field=="updated")
	{
		return now();
	}
				if($table=="questions" && $field=="now")
	{
		return now();
	}
				if($table=="questions" && $field=="updated")
	{
		return now();
	}
				if($table=="users" && $field=="now")
	{
		return now();
	}
				if($table=="users" && $field=="updated")
	{
		return now();
	}
				if($table=="registrations" && $field=="now")
	{
		return now();
	}
				if($table=="registrations" && $field=="updated")
	{
		return now();
	}
				if($table=="sub_category" && $field=="create_date")
	{
		return now();
	}
				if($table=="sub_category" && $field=="last_update")
	{
		return now();
	}
	return "";
